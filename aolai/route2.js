...
function draw_globalmap() {
    ...
    foo_x = globalMap.mapCenterX;
    foo_y = globalMap.mapCenterY;
    line_space = 1;
    for (var i = 0; i < 195; i++) {
        foo_x = globalMap.mapCenterX + (Math.sin(i/3) * line_space) * imgStatus.scale * i;
        foo_y = globalMap.mapCenterY + (Math.cos(i/3) * line_space) * imgStatus.scale * i;
        // console.log(foo_x + "," + foo_y);
